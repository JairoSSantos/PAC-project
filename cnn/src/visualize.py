...
def plot_training(model, clear=False, ranking=False):
    ...
    img = model.x_test[j, ..., 0]
    pred = y_pred_test[j, ..., 0]
    H, W = img.shape

    logs = model.get_logs()
    best_epoch = logs.epoch[np.argmin(logs.val_loss)]
    n_metrics = len(model.metrics) - 1

    # ==================== Figure config ====================
    fig = plt.figure(figsize=(20, 10))
    main_grid = GridSpec(3, 4, figure=fig)
    metrics_grid = main_grid[-1, :].subgridspec(1, n_metrics)

    axs = {
        'seg': fig.add_subplot(main_grid[:-1, 0]),
        'prec': fig.add_subplot(main_grid[:-1, -1]),
        'loss': fig.add_subplot(main_grid[:-1, 1:-1]),
    }

    # ==================== Segmentation ====================
    plot_label(img, pred >= 0.5, ax=axs['seg'])
    axs['seg'].contour(
        *np.meshgrid(np.arange(W), np.arange(H)), 
        pred,
    ...
    axs['loss'].vlines(best_epoch, label=f'best epoch: {best_epoch}', linestyle='dashed', color='k')
    ...
    # ==================== Precision ====================
    axs['prec'].scatter(np.mean(model.y_train, axis=(-1, -2, -3)), area_pred_train, alpha=0.7, label='training data')
    axs['prec'].scatter(np.mean(model.y_test, axis=(-1, -2, -3)), area_pred_test, alpha=0.7, label='validation data')
    ...
    for i, metric in enumerate(model.metrics):
        ...
        ax.plot(logs.epoch, logs[metric.name], label=metric.name)
        ax.plot(logs.epoch, logs[f'val_{metric.name}'], label=f'validation {metric.name}')
        ax.set_xlabel('epoch')
    ...
    if ranking != False: 
        display(HTML(logs.sort_values('val_loss', ascending=False).head(int(ranking)).to_html()))

class TrainingBoard(Callback):
    def __init__(self, model, period, ranking):
        super().__init__()
        self.model = model
        self.period = period
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch%self.period == 0:
            plot_training(self.model, clear=True, ranking=self.ranking)