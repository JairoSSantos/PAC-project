...
def chanvese(I, init_mask, max_its=200, alpha=0.2,
    ...
    while (its < max_its and not stop):
        ...
        if len(idx) > 0:
            ...
            new_mask = phi <= 0
            c = convergence(prev_mask, new_mask.astype(int), thresh, c)
...
img = color.rgb2gray(np.array(Image.open(os.path.join(images_path, '0.jpg'))))

d = 50
x, y = np.meshgrid(np.arange(0, img.shape[0]), np.arange(0, img.shape[1]))
mask = np.where((x - img.shape[0]//2)**2 + (y - img.shape[1]//2)**2 < d**2, 1, 0)
